...
function CFAmixEFA(Y::Matrix{Float64}, q::Int64,  K::Int64, maxIter::Int64, X::Matrix{Float64}, maxNumRestart::Int64, mu_hat::Int64=1,L_str=nothing)
    ...
    while code_error != 8.0
        ...
        # Starting values
        start_vals = starting_val(n,p,q,K,X,L_str)
...
function EM(Y::Matrix{Float64},X::Matrix{Float64},q::Int64, K::Int64, p::Int64, n::Int64, ncov::Int64, maxIter::Int64, Lambda_1::Matrix{Float64}, Lambda_2::Matrix{Float64}, Theta_delta::Diagonal{Float64, Vector{Float64}}, Psi_delta::Diagonal{Float64, Vector{Float64}}, Phi::Matrix{Float64}, kappa::Vector{Float64}, betas::Vector{Float64}, L_str::Matrix, mu::Vector{Float64}, nu::Vector{Float64}, mu_hat::Int64)
    ...
    while diff_llik>dtol && t<=maxIter && sum_diff_tf
            ...
            Psi_delta = Psi_delta_mstep(n,Lambda_2,ESyy0,Exiy0,Ek0)
            ...
            if sum(Theta_delta.<0) > 0 || det(Psi_delta) < 0
                code_error = 0.0
                return vcat(vec(fill(NaN,p,q)), vec(fill(NaN,p,K)), vec(diag(fill(NaN,p,p))), vec(diag(fill(NaN,p,p))), vec(fill(NaN,q,q)), vec(fill(NaN,n)), NaN, vec(fill(NaN,n)), vec(fill(NaN,ncov)), vec(fill(NaN,q)), vec(fill(NaN,K)), code_error, sum_diff_tf, t-1) 
...
# Starting values
function starting_val(n::Int64, p::Int64, q::Int64, K::Int64, X::Matrix,L_str=nothing)
